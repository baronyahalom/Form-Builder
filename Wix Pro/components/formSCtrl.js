angular.module('formBuildApp')
    .controller('formSCtrl', ['$rootScope','$routeParams', '$scope', '$route', '$location', '$http', function ($rootScope,$routeParams, $scope, $route, $location, $http) {
        let self = this;
        ...
        self.getFormbyID = function () {
            ...
            $http.get(serverUrl + "forms/" + id)
                .then(function (response) {
                   $scope.fullFeilds = response.data;             
                   console.log($scope.fullFeilds)
                   self.getFormName(id)
                }, function (response) {
                });
        };
